...
export function useVisualConfig() {
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const geminiHeaderDefaults =
        asRecord(parsed['gemini-header-defaults']) ??
        asRecord(parsed['gemini-cli-header-defaults']);
      ...
      const newValues: VisualConfigValues = {
        ...
        geminiHeaderUserAgent:
          typeof geminiHeaderDefaults?.['user-agent'] === 'string'
            ? geminiHeaderDefaults['user-agent']
            : '',
        openAICompatHeaderUserAgent:
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        // FORK[provider-headers]: preserve upstream's managed-map behavior for fork defaults.
        if (dirtyFields.has('geminiHeaderUserAgent')) {
          ensureMapInDoc(doc, ['gemini-header-defaults']);
          setStringInDoc(
            doc,
            ['gemini-header-defaults', 'user-agent'],
            values.geminiHeaderUserAgent
          );
          deleteIfMapEmpty(doc, ['gemini-header-defaults']);
          if (docHas(doc, ['gemini-cli-header-defaults'])) {
            doc.deleteIn(['gemini-cli-header-defaults']);
          }
        }
        if (dirtyFields.has('openAICompatHeaderUserAgent')) {
          ensureMapInDoc(doc, ['openai-compatibility-header-defaults']);
          setStringInDoc(
            doc,
            ['openai-compatibility-header-defaults', 'user-agent'],
            values.openAICompatHeaderUserAgent
          );
          deleteIfMapEmpty(doc, ['openai-compatibility-header-defaults']);
        }
        if (dirtyFields.has('kimiHeaderUserAgent')) {
          ensureMapInDoc(doc, ['kimi-header-defaults']);
          setStringInDoc(doc, ['kimi-header-defaults', 'user-agent'], values.kimiHeaderUserAgent);
          deleteIfMapEmpty(doc, ['kimi-header-defaults']);
        }
        if (dirtyFields.has('antigravityHeaderUserAgent')) {
          ensureMapInDoc(doc, ['antigravity-header-defaults']);
          setStringInDoc(
            doc,
            ['antigravity-header-defaults', 'user-agent'],
            values.antigravityHeaderUserAgent
          );
          deleteIfMapEmpty(doc, ['antigravity-header-defaults']);
        }
